# -*- coding: utf-8 -*-
"""
Created on Wed Jun  6 15:27:16 2018
...
for l1 in L1:
    L2 = np.arange(l1, 1+step_size, step_size)
    if L2[-1]==1+step_size:
        L2 = np.delete(L2, -1)
    for l2 in L2:
        if l1==0.0 and l2==0.0:
            continue
        traffic_loads['{:.2f}_{:.2f}'.format(l1, l2)] = [l1, l2]
...
num_stat = []
thp_stat = []
for k, v in tqdm(traffic_loads.items()):
...
#    print k
    ...
    for i in range(num_repetition):
        t = Traffic(num_pods=num_pods, max_pod_connected=min(int(v[1]*num_pods), num_pods), 
                    min_pod_connected=max(int(v[0]*num_pods), 0), 
                    capacity_choices=data_rate_choice,
                    capacity_probs=[0.90, 0.10])
        t.generate_traffic()
